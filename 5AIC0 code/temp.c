...
CAN_SYMBOL * dec_to_bin(uint64_t number) {
    ...
    CAN_SYMBOL * binary = (CAN_SYMBOL*) malloc((i+3)*sizeof(CAN_SYMBOL));           // i+3 instead of i+1, because apparently im overwriting un-malloced space which caused the free() error
